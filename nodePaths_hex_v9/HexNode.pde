class HexNode {
  ...
  void display() {
    ...
    if (activeSize > 1) {
      ...
      scale(.95);
      if (mainStream) {
        tint(255, map(activeSize, 0, 50, 0, 200));
      } else {
        tint(0, 100, 200, map(activeSize, 0, 50, 0, 150));  
      }
      image(hexImage, 0, 0);
      popMatrix();
      /*
  ...
  void advanceNext() {
    if (active) {   
      ...
      hexNodes.queueNode(followerId0, true);
      hexNodes.queueNode(followerId1, true);
      ...
      for (int i=0; i<6; i++) {
         nextId = neighbors[i];
         if ( nextId == followerId0 || nextId == followerId1) {
         } else {
           if (random(0, 10) > 9.2) hexNodes.queueNode(nextId, false); 
         }
      }
      active = false;
    }
